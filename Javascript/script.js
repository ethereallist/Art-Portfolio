...
// Asignar un clic a todos los puntos, click en punto
// Saber la posición de ese punto
// Aplicar un transform translateX al grande
// Quitar la clase activo de los puntos
// Añadir clase activo al punto en el que se hace click

punto.forEach( (cadaPunto , i) => {
    punto[i].addEventListener ('click', ()=> {
        let posicion = i
        // Cuando la posición es 0 transformX es 0
        // Cuando la posición es 1 transformX es -25%
        // operación = posición * -25
        let operacion = posicion * -50
        grande.style.transform = `translateX($ {operacion} %)`
    }) 
}) 